...
const sudokuSolver = (sudokuGrid = []) => {
    ...
    const rowAndColumnIndicesWithinEachThreeByThreeSubgridWithinSudokuGrid = [
    ...
    while (numberOfBlankSquaresForWhileLoopCounter > 0) {
        for (var i = currentRow; i < 9; i++) { // We set our index variables to current row and current column since we will update this values and break the loops if performBackTracking is set to true as part of our backtracking process in order to undo combinations try another combination with a different value from a previously added value that we may be removing if we reach a dead end 
            for (var j = currentColumn; i < 9; j++) { //From the current row and current column variables we will keep iterating through the grid from top to bottom first left to right second 
                currentColumn = j;
                if (sudokuGrid[currentRow][currentColumn] == 0) {
                    findWhatEnteredNumbersIntoRowAndColumnWillLeadToDeadEndEnteredNumbersCombinationsIfThatCanHappen(unavailableEnteringNumbersChoices, currentRow, currentColumn, deadEndEnteredNumbersCombinations, currentEnteredNumbersCombination);
                    ...
                    findNumbersWithinowColumnOrThreeByThreeBlockWhereWeFindOurselves(unavailableEnteringNumbersChoices, sudokuGrid, currentRow, currentColumn, rowAndColumnIndicesWithinEachThreeByThreeSubgridWithinSudokuGrid);
                    ...
                    for (var i = 0; i < possibleNumberChoices.length; i++) {
                        if (unavailableEnteringNumbersChoices.includes(possibleNumberChoices[i])) {
                            continue;
                        }
                        else {
                            numbersThatWeCanEnterIntoThisBlankSquare.push(possibleNumberChoices[i]);
                        }
                    }
                    ...
                    sudokuGrid[currentRow][currentColumn] = newNumberToBeAdded;
                    newlyAddedFilledSquare[0] = currentRow; //And then we add a new square with our newly added number to our currentEnteredNumbersCombination array which we use to compare against dead end combinations 
                    newlyAddedFilledSquare[1] = currentColumn;
                    newlyAddedFilledSquare[2] = newNumberToBeAdded;
                    currentEnteredNumbersCombination.push(newlyAddedFilledSquare);
...
const findNumbersWithinowColumnOrThreeByThreeBlockWhereWeFindOurselves = (unavailableEnteringNumbersChoices, sudokuGrid, currentRow, currentColumn, rowAndColumnIndicesWithinEachThreeByThreeSubgridWithinSudokuGrid) => {
    var numbersWithinRow = [];
    var numbersWithinColumn = [];
    var numbersWithinThreeByThreeBlock = [];
    var rowColumnIndicesFromThreeByThreeBlockWeFindOurselvesAt = [];
    for (var i = 0; i < 9; i++) {
...
const findThreeByThreeIndicesBlockWithinSudokuGridWhereCurrentRowAndCurrentColumnAreAt = (currentRow, currentColumn, rowAndColumnIndicesWithinEachThreeByThreeSubgridWithinSudokuGrid) => {
    ...
    var rowColumnWithinThisSpecificThreeByThreeGridWithinSudokuGridTuple = [0, 0];
...
const removeSquareValueFromSudokuAndSquareRowColumnAndValueFromCurrentEnteredNumbersCombination = (sudokuGrid, currentEnteredNumbersCombination, backtrackingTargetPoint) => {
...
const returnRandomPreviouslyFilledSquareWithinCurrentSquaresRowColumnOr3x3BlockAsARowColumnValueTupleToEmptyItAndRefillIt = (currentRow, currentColumn, sudokuGrid, currentEnteredNumbersCombination, rowAndColumnIndicesWithinEachThreeByThreeSubgridWithinSudokuGrid) => {
    ...
    //the second case. 
    var threeByThreeBlockWithinThreeByThreeBlocksRowsAndColumnsIndicesWithinSudokuGridAtThisIndex = 0;
    var rowColumnIndicesWithinThreeByThreeBlockWhereCurrentRowAndColumnAreLocated = [0, 0];
    var rowColumnWithinThisSpecificThreeByThreeGridWithinSudokuGridTuple = [0, 0];
    var previouslyFilledSquaresWithinCurrentSquareRowColumnOr3x3BlockArray = [];
    var previouslyFilledSquareAsRowColumnValueTuple = [0, 0, 0];
...
const findBlankSquaresAmountCounterForMainWhileLoop = (sudokuGrid) => {
    var blankSquaresCount = 0;
    var currentRow = 0;
    var currentColumn = 0;
    for (var i = 0; i < 9; i++) {
        currentRow = i;
...
const addUnavailableEnteringNumbersChoicesFromGivenPoolToUnavailableEnteringNumbersChoices = (unavailableEnteringNumbersChoices, unavailableEnteringNumbersChoicesFromGivenPool) => {
    ...
    unavailableEnteringNumbersChoices = [...new Set(unavailableEnteringNumbersChoices)];
};
//First part of our algorithm as explained in the top comment 
const findWhatEnteredNumbersIntoRowAndColumnWillLeadToDeadEndEnteredNumbersCombinationsIfThatCanHappen = (unavailableEnteredNumbersChoices, currentRow, currentColumn, deadEndEnteredNumbersCombinations, currentEnteredNumbersCombination) => {
    var deadEndEnteredNumbersCombinationIndex = 0;
    var enteredNumberIndexWithindeadEndEnteredNumbersCombination = 0;
    var potentialMatchingEnteredNumberAtRowAndColumnWithinDeadEndCombinationWithCurrentRowAndColumn = [0, 0, 0];
    var matchingEnteredNumberAtRowAndColumnWithinDeadEndCombinationWithCurrentRowAndColumn = [0, 0, 0];
    var deadEndEnteredNumbersCombination = [];
    var unavailableEnteringNumbersChoicesFromDeadEndCombinations = [];
    if (deadEndEnteredNumbersCombinations.length > 0) {
        ...
        addUnavailableEnteringNumbersChoicesFromGivenPoolToUnavailableEnteringNumbersChoices(unavailableEnteredNumbersChoices, unavailableEnteringNumbersChoicesFromDeadEndCombinations);
    }
};
